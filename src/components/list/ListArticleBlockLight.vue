<template>
  <div class="listArticleBlockLight">
    <figure v-if="image" class="listArticleBlockLight__figure">
      <a :href="`/external/${get(article, [ 'name' ])}`" target="_blank">
        <img v-lazy="image" :alt="get(article, [ 'title' ])" />
      </a>
    </figure>
    <div class="listArticleBlockLight__text" :class="{ noImg: !image }">
      <h2><a :href="`/external/${get(article, [ 'name' ])}`" target="_blank" v-text="get(article, [ 'title' ])"></a></h2>
      <p><a :href="`/external/${get(article, [ 'name' ])}`" target="_blank" v-text="$_listArticleLight_getBrief(article)"></a></p>
    </div>
  </div>
</template>
