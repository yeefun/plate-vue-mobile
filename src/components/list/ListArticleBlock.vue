<template>
  <div class="listArticleBlock" :class="{ noHoverEffect: removeHoverEffect }" >
    ...
    <template v-else-if="articleType === 'topic'">
      <figure class="listArticleBlock__figure">
        <a :href="`/topic/${get(article, 'id')}`" target="_blank" @click="sendGaClickEvent('listing', 'latest')">
          <LazyImage :src="getImage(article, 'mobile')" :caption="get(article, 'name')" />
        </a>
      </figure>
      <div class="listArticleBlock__content">
        <h2><a :href="`/topic/${get(article, 'id')}`" target="_blank" @click="sendGaClickEvent('listing', 'latest')" v-text="get(article, 'name')"></a></h2>
        <p><a :href="`/topic/${get(article, 'id')}`" target="_blank" @click="sendGaClickEvent('listing', 'latest')" v-text="getBrief(article, 45)"></a></p>
      </div>
    </template>
    <template v-else>
      <figure class="listArticleBlock__figure">
      ...
      <div class="listArticleBlock__content">
        ...
        <h2><a :href="getHref(article)" target="_blank" @click="sendGaClickEvent('listing', 'latest')" v-text="get(article, 'title')"></a></h2>
