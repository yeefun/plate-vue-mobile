...
node {
    ...
    stage('Pre-build Setup') {
        ...
        try {
            ...
            sh("git clone -b master https://github.com/mirror-media/plate-vue-mobile-docker.git")

            sh("gcloud source repos clone default --project=mirrormedia-1470651750304")
            sh("cp default/plate-vue-mobile/config.js plate-vue-mobile-docker/")
            sh("cp default/keystone/gcskeyfile.json plate-vue-mobile-docker/")
    ...
    stage("Deploy") {
        try {
            // Deploy to dev
            sh("kubectl set image deploy/vue-mobile--deployment plate-vue-mobile=${imageTag}:${slack_user}_${build_time}")
            // Watch until rollout success
            sh("kubectl rollout status deployment/vue-mobile-deployment -w")
